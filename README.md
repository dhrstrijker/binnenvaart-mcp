...
| `tide_departure_window` | Non-binding departure-window assessment for the focused tide/current question: broad planning anchors like Europoort, Rotterdam, Amsterdam, Antwerp, Harlingen or Terschelling; draft + margin; route/depth evidence where available; typed RWS/Waterinfo Vlaanderen tide and water-level context; arrival constraints when supplied; and explicit blockers when official current direction/speed, high-water extrema or depth basis is missing. Returns `verdict`, `summary`, `route_assumptions`, ranked `candidate_windows` with section score/timeline, `current_assessment`, `depth_assessment`, `sources`, `bronregels` and `datagaten`. |
...
The focused `tide_departure_window` engine also uses official public tide/current context from Rijkswaterstaat Waterinfo/DDAPI20 and Waterinfo Vlaanderen KiWIS where available. KiWIS H-series are classified as forecast, measurement, threshold, statistic, status or unknown before use; threshold/status/statistic series are not used as passage water-level values. KiWIS `Q`/debiet is hydrological context only, and KiWIS `v`/velocity without a paired current-direction series is not enough to classify current as with or against the route. These sources remain provenance/context for a non-binding planner and are not endorsements. This project is independent — **not affiliated with or endorsed by EuRIS, Rijkswaterstaat or Waterinfo Vlaanderen** — and reads only public/open data (no GDPR-protected personal data).
