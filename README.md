...
| `tide_departure_window` | Non-binding departure-window assessment for the focused tide/current question: broad planning anchors like Europoort, Rotterdam, Amsterdam, Antwerp, Harlingen or Terschelling; draft + margin; route/depth evidence where available, including official RWS DDAPI20 `VAARDTE` vaardiepte where matched and fresh; typed RWS/Waterinfo Vlaanderen tide and water-level context; arrival constraints when supplied; and explicit blockers when official current direction/speed, high-water extrema or depth basis is missing. Returns `verdict`, `summary`, `route_assumptions`, ranked `candidate_windows` with section score/timeline, `current_assessment`, `depth_assessment`, `sources`, `bronregels` and `datagaten`. |
...
`tide_departure_window` also returns a structured non-binding `verdict` because departure-window questions need one backend status for the client to explain. Candidate windows are ranked from section passagetimes, current/depth blockers and optional `arrival_by`; each score can include `arrival_constraint`, estimated arrival, latest feasible departure and controlling sections. It still stays source-grounded: water level is not compared directly with draft, and missing current direction/current speed is a blocker rather than an estimate.

## Data source & attribution

Core inland-shipping data comes from **[EuRIS](https://www.eurisportal.eu)** (European River Information Services) open-data APIs — Hydrometeo (water levels and least sounded depth), the RIS Index (objects), RouteCalculatorV2 (voyages) and Notices to Skippers. Per EuRIS's terms, the data is **incorporated from EuRIS (eurisportal.eu), Copyright © EuRIS**.

The focused `tide_departure_window` engine also uses official public tide/current/depth context from Rijkswaterstaat Waterinfo/DDAPI20 and Waterinfo Vlaanderen KiWIS where available. RWS DDAPI20 `VAARDTE` is treated as official observed navigable depth only when the catalog location matches the route section and a fresh value can be fetched near the section passagetime; public `WATDTE` beach/recreation water-depth series are not used as route depth evidence. KiWIS H-series are classified as forecast, measurement, threshold, statistic, status or unknown before use; threshold/status/statistic series are not used as passage water-level values. KiWIS `Q`/debiet is hydrological context only, and KiWIS `v`/velocity without a paired current-direction series is not enough to classify current as with or against the route. These sources remain provenance/context for a non-binding planner and are not endorsements. This project is independent — **not affiliated with or endorsed by EuRIS, Rijkswaterstaat or Waterinfo Vlaanderen** — and reads only public/open data (no GDPR-protected personal data).
