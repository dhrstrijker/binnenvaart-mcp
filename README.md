...
The focused `tide_departure_window` engine also uses official public tide/current context from Rijkswaterstaat Waterinfo/DDAPI20 and Waterinfo Vlaanderen KiWIS where available. KiWIS H-series are classified as forecast, measurement, threshold, statistic, status or unknown before use; threshold/status/statistic series are not used as passage water-level values. These sources remain provenance/context for a non-binding planner and are not endorsements. This project is independent — **not affiliated with or endorsed by EuRIS, Rijkswaterstaat or Waterinfo Vlaanderen** — and reads only public/open data (no GDPR-protected personal data).
